...
public class Observations {
    ...
    public static String weatherReport() {
        if (stormyDefinition()) {
            ...
            return weatherReport;
        } else if (risingFallingUnSettledSteady("pressure", "hourly").equals("unsettled")&& risingFallingUnSettledSteady("temperature", "hourly").equals("falling")){
            weatherReport = getWeatherReport() +
                    "Little weird out there for mAH d00ds. Keep an eye on the sky and be flexible but I'd probably take it inside.";
        return weatherReport;
        } else if (risingFallingUnSettledSteady("pressure", "hourly").equals("unsettled")&& risingFallingUnSettledSteady("temperature", "hourly").equals("rising")){
            weatherReport = getWeatherReport() +
            ...
            return weatherReport;
        } else if (risingFallingUnSettledSteady("pressure", "hourly").equals("unsettled")&& risingFallingUnSettledSteady("temperature", "hourly").equals("unsettled")){
            weatherReport = getWeatherReport() +
                    "Little weird out there for mAH d00ds. Cup of tea while you wait a bit?";
            return weatherReport;
    }else if (risingFallingUnSettledSteady("pressure", "hourly").equals("fairly steady") && risingFallingUnSettledSteady("wind", "hourly").equals("fairly steady")){
            weatherReport = getWeatherReport() +
...
// definitions
    private static String risingFallingUnSettledSteady(String column, String table) {
        ...
        if (readingThree > readingTwo && readingTwo > readingOne){
            ...
            return "falling";
        }else if (Math.abs(readingOne-readingThree) <= 10){
    ...
    private static boolean deterioratingDefinition(){
        return risingFallingUnSettledSteady("pressure", "hourly").equals("falling")
                && (risingFallingUnSettledSteady("wind", "hourly").equals("rising") ||
                risingFallingUnSettledSteady("temperature", "hourly").equals("falling"));
    }
}